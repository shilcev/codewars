...
func solution(_ string: String) -> Int {
     let symbol: [Character:Int] = ["I": 1, "V": 5, "X": 10, "L": 50, "C": 100, "D": 500, "M": 1000]
     var count = 0
     let transform: (Character) -> Int = {
         let res = symbol[$0]!
         if res > count { count = res }
         return res < count ? -res : res
     }
...
func thirt(_ n: Int) -> Int {
   let result = String(n).reversed().compactMap{ Int("\($0)") }.enumerated().map {
        $0.element * (Int(pow(Double(10), Double($0.offset))) % 13)}.reduce(0, +)
    if result != n { return thirt(result) }
    return result
}
